...
export default function Tracker({ snapshot, mode, onSnapshot, onError, onBeforeOperation, onOpenEntry }: TrackerProps) {
  ...
  const completeCount = visibleRuns.filter(run => run.status === 'complete').length;
  ...
  return <main className="tracker-main"><div className="breadcrumb-bar"><span><GitBranch size={15} />Organization<span className="breadcrumb-slash">/</span><strong>Experiment tracker</strong></span><span className="quiet-label">One step at a time</span></div><div className="tracker-content"><div className="page-heading tracker-heading"><div><span className="eyebrow">A little forward motion</span><h1>Experiment tracker<span className="heading-spark" aria-hidden="true">✦</span></h1><p>From a good question to a completed experiment.</p></div><label className="tracker-filter">View notebook<select aria-label="Filter tracker by notebook" value={notebookFilter} onChange={event => setNotebookFilter(event.target.value)}><option value="all">All notebooks</option>{notebooks.map(notebook => <option key={notebook.id} value={notebook.id}>{notebook.name}</option>)}</select></label></div><div className="tracker-summary"><div className="tracker-summary-icon"><Sparkles size={20} /></div><div><strong>Every step counts.</strong><p role="status">{completeCount} of {visibleRuns.length} experiments complete. There’s room for what’s next.</p></div><span className="soft-badge">{mode === 'demo' ? 'Demonstration' : 'Saved'}</span></div><div className="tracker-board">{statusOptions.map(status => { const items = visibleRuns.filter(run => run.status === status.id); const Icon = statusIcons[status.id]; return <section key={status.id} className={`tracker-column ${status.id}`} aria-labelledby={`tracker-${status.id}`}><div className="tracker-column-header"><span className="tracker-status-icon"><Icon size={17} /></span><h2 id={`tracker-${status.id}`}>{status.name}</h2><span className="tracker-count" aria-label={`${items.length} experiments`}>{items.length}</span></div><p className="tracker-column-description">{status.description}</p><div className="tracker-cards">{items.map(run => { const notebook = notebooks.find(item => item.id === run.notebookId); return <article className="tracker-card" key={run.id}><span className="tracker-notebook"><span className={`notebook-dot ${notebook?.color ?? 'sage'}`} />{notebook?.name ?? 'Unknown notebook'}</span><button className="tracker-entry-link" onClick={() => onOpenEntry(run.notebookId, run.id)}><h3>{run.title}</h3><ArrowUpRight size={15} /></button><span className="tracker-entry-code">{entryCode(run)}</span><p>{run.author}</p><div className="tracker-card-footer"><span className="tracker-avatar" title={run.author}>{run.author.split(' ').map(name => name[0]).join('')}</span><select id={`status-${run.id}`} aria-label={`Status for ${run.title}`} value={run.status} onChange={event => void changeStatus(run.id, event.target.value as EntryStatus)}>{statusOptions.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></div></article>; })}{!items.length && <div className="tracker-empty"><Icon size={24} /><strong>A little breathing room.</strong><span>No experiments in this column.</span></div>}</div></section>; })}</div><p className="tracker-footnote">Experiment status is saved with each run and remains after restart.</p></div></main>;
}
